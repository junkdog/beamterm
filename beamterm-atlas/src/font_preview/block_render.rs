...
impl BlockRenderer {
    ...
    fn sample_coverage(&self, bitmap: &[u8], width: usize, height: usize, 
                      cell_x: usize, cell_y: usize, scale_x: f32, scale_y: f32) -> f32 {
    ...
    for y in 8..40 {
        for x in 4..28 {
            let distance_from_edge = std::cmp::min(
                std::cmp::min(x - 4, 28 - x),
                std::cmp::min(y - 8, 40 - y)
            );
