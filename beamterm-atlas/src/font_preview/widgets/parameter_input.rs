...
impl ParameterInputState {
    pub fn new(value: f32, min: f32, max: f32, step: f32, precision: usize) -> Self {
        Self { value, focused: false, min, max, step, precision }
    }
...
impl<'a> StatefulWidget for ParameterInput<'a> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let label_style = if state.focused {
            self.theme.parameter_label.fg(self
                .theme
                .border_focused
                .fg
        ...
        Paragraph::new(Line::from(vec![Span::styled(value_text, value_style)]))
            .render(chunks[1], buf);

        // Render increment button
        let inc_style = if state.focused { self.theme.button_focused } else { self.theme.button };

        Paragraph::new(Line::from(vec![Span::styled("[+]", inc_style)])).render(chunks[2], buf);

        // Render decrement button
        let dec_style = if state.focused { self.theme.button_focused } else { self.theme.button };

        Paragraph::new(Line::from(vec![Span::styled("[-]", dec_style)])).render(chunks[3], buf);
    }
}
