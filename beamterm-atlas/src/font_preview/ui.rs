...
impl UI {
    ...
    fn render_controls(&mut self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_stateful_widget(font_selector, control_chunks[0], &mut self.font_selector_state);
        
        // Parameter inputs
        let font_size_input = ParameterInput::new("Size", "pt", &self.theme);
        frame.render_stateful_widget(font_size_input, control_chunks[1], &mut self.font_size_state);
        
        let line_height_input = ParameterInput::new("Line Height", "x", &self.theme);
        frame.render_stateful_widget(line_height_input, control_chunks[2], &mut self.line_height_state);
        
        // Separator
        let separator = Paragraph::new("─── Decorations ───────")
            .style(self.theme.parameter_label);
        frame.render_widget(separator, control_chunks[4]);
        
        let underline_pos_input = ParameterInput::new("U.Position", "%", &self.theme);
        frame.render_stateful_widget(underline_pos_input, control_chunks[5], &mut self.underline_position_state);
        
        let underline_thick_input = ParameterInput::new("U.Thickness", "%", &self.theme);
        frame.render_stateful_widget(underline_thick_input, control_chunks[6], &mut self.underline_thickness_state);
        
        let strike_pos_input = ParameterInput::new("S.Position", "%", &self.theme);
        frame.render_stateful_widget(strike_pos_input, control_chunks[7], &mut self.strikethrough_position_state);
        
        let strike_thick_input = ParameterInput::new("S.Thickness", "%", &self.theme);
        frame.render_stateful_widget(strike_thick_input, control_chunks[8], &mut self.strikethrough_thickness_state);
    }
    ...
    fn update_focus(&mut self) {
        ...
        self.strikethrough_position_state.set_focused(false);
        self.strikethrough_thickness_state.set_focused(false);
        ...
        match self.focused_widget {
            ...
            FocusedWidget::StrikethroughPosition => self.strikethrough_position_state.set_focused(true),
            FocusedWidget::StrikethroughThickness => self.strikethrough_thickness_state.set_focused(true),
            _ => {}
        }
    }
    ...
    fn update_font_display(&mut self) -> Result<()> {
        ...
        if let Some(font_generator) = &mut self.current_font_generator {
            ...
            debug!("Updated {} glyph variants - protocols will be created fresh each frame", 
    ...
    fn render_symbol_with_style(
        ...
        if let Some(ref mut generator) = self.current_font_generator {
            ...
            let glyph_bitmap = generator.rasterize_symbol(symbol, style, bounds)
                .debug_checkered();
            debug!(
                symbol = %symbol,
                style = ?style,
            ...
            self.status_message = Some(format!("Rasterized '{symbol}' {style:?} ({}x{})",
